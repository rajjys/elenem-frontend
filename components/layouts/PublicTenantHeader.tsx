...
export const PublicTenantHeader = () => {
  ...
  return (
    <>
      <header
        className={`w-full border-b border-${secondaryColor}-200 bg-line/95 backdrop-blur-md z-50 transition-transform duration-300
        ...
        <div className="mx-auto max-w-7xl px-3 lg:px-6 flex h-12 lg:h-16 items-center justify-between gap-4">
            ...
            <nav className="hidden md:flex items-center gap-6 text-base">
              {mainLinks.map((link) => {
                  ...
                  return (
                    <div className="relative" key="plus" ref={moreRef}>
                      <button
                        ...
                        className={`flex flex-col items-center text-xs md:text-sm text-ink-muted hover:text-ink hover:text-${primaryColor}-700 cursor-pointer`}>
                      ...
                      {moreOpen && (
                        <div className="absolute right-0 mt-2 py-4 text-xs md:text-sm rounded-md border-b bg-surface shadow-lg z-40">
                          {moreLinks.map((ml) => (
                            <Link
                              key={ml.href}
                              href={ml.href}
                ...
                return (
                  ...
                  isActive ? `font-semibold text-${primaryColor}-700` : "text-ink-muted"
                }`}
                  >
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                href="/"
                className="hidden md:inline text-sm font-medium px-3 py-1.5 rounded-md border border-line hover:bg-surface-sunk"
              ...
              <Link
                href="/"
                className="md:hidden inline-flex items-center justify-center p-2 rounded-full border border-line hover:bg-surface-sunk"
              >
      ...
      {/* Mobile Bottom Nav */}
      <nav className="fixed bottom-0 left-0 right-0 z-40 bg-surface border-t border-line md:hidden">
          ...
          {mainLinks.map((link) => {
            ...
            if (link.label === "Plus") {
              return (
                <button
                  ...
                  className="flex flex-col items-center text-xs text-ink-muted"
                >
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`flex flex-col items-center gap-1 text-xs ${isActive ? `font-semibold text-${primaryColor}-700` : "text-ink-muted"}`}
              >
          ...
          <nav className="flex flex-col divide-y">
            {moreLinks.map((ml) => (
              <Link
                key={ml.href}
                href={ml.href}
                className="px-4 py-3 text-base hover:bg-surface-sunk"
